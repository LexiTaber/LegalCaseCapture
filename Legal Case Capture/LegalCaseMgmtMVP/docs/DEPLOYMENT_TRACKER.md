...
| Component | API Name | Status | Notes |
|---|---|---|---|
| Legal Case Mgmt - General Access | `LegalCaseMgmt_GeneralAccess` | Deployed (Production) | **Local source reconciled with `KHRCProd` on 2026-08-05.** During UAT, non-owner users got "insufficient access rights on cross-reference id" creating `LegalCaseNote__c` records on cases they didn't own — `viewAllRecords=true` on `LegalCase__c` grants read but not the edit access a Master-Detail child insert requires on its parent, and all 95 `LegalCase__c` records are owned by a single user (`taber@rfkhumanrights.org`), so this blocked every other tester. Fixed by setting `modifyAllRecords=true` on `LegalCase__c` (Setup UI auto-enabled `allowDelete` alongside, standard behavior when checking Modify All). This is a blanket cross-case edit grant, not per-case ownership/sharing — worth revisiting post-UAT. Local source also picked up several previously-ungranted `LegalCase__c` field permissions (`Case_Number__c`, `Circuit__c`, `Filing_Date__c`, `Total_Hours_Logged__c`, `US_Team_Assigned__c`, `Website_Overview__c`) added in the same org session. |
